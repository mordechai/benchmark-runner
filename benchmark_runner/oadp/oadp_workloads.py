...
class OadpWorkloads(WorkloadsOperations):
    ...
    def __init__(self):
        ...
        self.__oadp_scenario_name = 'restore-100pod-backup-vsm-pvc-util-4-1-0-cephrbd-6g' #self._environment_variables_dict.get('oadp_scenario','')
    ...
    def config_dpa_for_cephfs_shallow(self, enable):
        """
    ...
    def run_workload(self):
        ...
        if test_scenario['args']['plugin'] == 'vsm':
            ...
            if expected_sc == 'ocs-storagecluster-cephfs-shallow':
                self.config_dpa_for_cephfs_shallow(enable=True)
            else:
                self.config_dpa_for_cephfs_shallow(enable=False)
        else:
